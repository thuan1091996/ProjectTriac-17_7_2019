...
interrupt [] void timer0_ovf_isr(void)
{
    ...
    */
    TCNT0=0xC8;    // 200 (0xC8) * 8 (prescaler) * 1/(8*10^6)= 200us 
    T_delay++;
    //------------------------------------------------------//
};
/*----------------------------------------------*/
void main(void)
{
    ...
    #asm("sei")
    Delay_200us(50);
    count_temp0 = EEPROM_Read(210);   //Load value from EEPROM
    Delay_200us(50);
    count_temp1 = EEPROM_Read(200);
    Delay_200us(50);
    while (1)
      {      
        unsigned char i=0;
...
void CheckSW(void)  //TEST DONE
{
    if(Mode==1)
    {
        if(!SW_GIAM)
        {
            if(count_temp1>0)       count_temp1--;
            else                    count_temp1=0;
            Delay_200us(1000);
           // #asm("cli");
            EEPROM_Write(200,count_temp1);
           // #asm("sei");
            
		}
        if(!SW_TANG)
        {
            if(count_temp1<99)      count_temp1++;
            else                    count_temp1=99;
            Delay_200us(1000);  
           // #asm("cli");
            EEPROM_Write(200,count_temp1);
    ...
    if(Mode==0)
    {
        if(!SW_GIAM)
        {
            if(count_temp0>0)       count_temp0--;     //Store count in EEPROM, EEPROM have only 100,000 write
            else                    count_temp0=0;
            Delay_200us(1000);
           // #asm("cli");
             EEPROM_Write(210,count_temp0);
           // #asm("sei");
        }
        if(!SW_TANG)
        {
            if(count_temp0<99)      count_temp0++;     //Store count in EEPROM, EEPROM have only 100,000 write
            else                    count_temp0=99;
            Delay_200us(1000);
           // #asm("cli");
             EEPROM_Write(210,count_temp0);
...
*/
    ...
    {              
            if(SW_ON==0)                    //Switch press toggle the power
            {
                    ...
                    Delay_200us(2500);      //Wait for bound
            }
    }
